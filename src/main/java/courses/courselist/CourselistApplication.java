...
public class CourselistApplication {
	...
	public CommandLineRunner courseDemo(CourseRepository repository, CategoryRepository crepository,
		...
		repository.save(new Course("Python ohjelmointi", "Kesä 2003", 5, 4, crepository.findByName("Programming").get(0)));
		repository.save(new Course("Business Intelligence", "ICB005AS2A-3003", 5, 5, crepository.findByName("Ict and Business").get(0)));
		repository.save(new Course("Liiketoimintaprosessit", "ICB005AS2A-3003", 5, 5, crepository.findByName("Ict and Business").get(0)));
		repository.save(new Course("Excel ja BI", "ANA001AS3A-3012", 5, 5, crepository.findByName("Ict and Business").get(0)));
		repository.save(new Course("Excel jatkokurssi", "ANA002AS2A-3009", 5, 5, crepository.findByName("Ict and Business").get(0)));
		repository.save(new Course("Professional English", "ENG001HH1AE-3026", 5, 3,crepository.findByName("Key Competencies").get(0)));
		repository.save(new Course("Tiimityö ja projektiosaaminen", "HRL001HH1A-3040", 5, 4,crepository.findByName("Key Competencies").get(0)));
		repository.save(new Course("Yrittäjyys ja liiketoiminta", "ENT001HH1A-3038", 5, 5,crepository.findByName("Key Competencies").get(0)));
		repository.save(new Course("Yrityksen talous", "ECO001HH1A-3045", 5, 5,crepository.findByName("Key Competencies").get(0)));
		repository.save(new Course("Asiakasymmärrys ja markkinointi", "MAR001HH1A-3115", 5, 3,crepository.findByName("Key Competencies").get(0)));
		repository.save(new Course("Ammatillinen viestintä", "OM001HH1A-3043", 5, 5,crepository.findByName("Key Competencies").get(0)));
		repository.save(new Course("ICT-valmiudet", "ICB001HH1A", 5, 5,crepository.findByName("Key Competencies").get(0)));
